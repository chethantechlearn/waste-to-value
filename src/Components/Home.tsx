      ...
      <main>
        {appData.sections.map((section) => (
          <section key={section.id} id={section.id} className="section-container">
            ...
            {showMetrics && section.metrics && section.metrics.length > 0 && (
              <div className="metrics-grid">
                {section.metrics.map((m: MetricData, idx: number) => (
                  <div key={idx} className="metric-card">
                    ...
                    {m.sourceUrl && (
                      ...
                      >
                        Source: {m.sourceName || 'Verified Reference'} ↗
                      </a>
                    )}
                  </div>
                ))}
            ...
            <div className="grid-2col">
              <div>
                {section.cards && section.cards.map((card: CardData, idx: number) => (
                ...
                {section.processSteps && (
                  <div className="process-grid">
                    {section.processSteps.map((step: ProcessStepData, idx: number) => (
                      <div key={idx} className="process-card">
                        <div className="process-number">{step.step}</div>
                        <h4 style={{ color: '#fff', margin: '6px 0' }}>{step.title}</h4>
                        <p style={{ color: '#a7f3d0', fontSize: '0.85rem' }}>{step.desc}</p>
                      </div>
                    ))}
                  </div>
                )}
...
function ImageViewer({ images }: { images: ImageData[] }) {
  ...
  return (
    <div className="image-viewer">
      ...
      {isMultiple && (
        <>
          <button className="slider-btn prev" onClick={() => setCurrentIndex((prev) => (prev - 1 + images.length) % images.length)}>
            &#10094;
          </button>
          <button className="slider-btn next" onClick={() => setCurrentIndex((prev) => (prev + 1) % images.length)}>
            &#10095;
          </button>
        </>
      )}
